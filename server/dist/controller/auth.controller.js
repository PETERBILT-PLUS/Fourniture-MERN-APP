...
export const register = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (userExist) {
            ...
            if (!DEVELOPMENT_MODE)
                throw new Error("the DEVELOPMENT_MODE is not availabe please check the .env file");
            if (adminExist) {
                const isMatch = yield bcrypt.compare(password, adminExist.password);
                if (!isMatch)
                    return res.status(401).json({ sucess: false, message: "Email ou mot de passe incorrect" });
